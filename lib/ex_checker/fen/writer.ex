defmodule ExChecker.FEN.Writer do
  ...
  def can_kingside_castle(board, :white) do
    case Enum.map([:e1, :h1], &Map.get(board, &1)) do
      [%{rank: :king, color: :white, has_moved: false}, %{rank: :rook, color: :white, has_moved: false}] -> true
      _ -> false
    end
  end
  def can_kingside_castle(board, :black) do
    case Enum.map([:e8, :h8], &Map.get(board, &1)) do
      [%{rank: :king, color: :black, has_moved: false}, %{rank: :rook, color: :black, has_moved: false}] -> true
      _ -> false
    end
  end
  ...
  def can_queenside_castle(board, :white) do
    case Enum.map([:a1, :e1], &Map.get(board, &1)) do
      [%{rank: :rook, color: :white, has_moved: false}, %{rank: :king, color: :white, has_moved: false}] -> true
      _ -> false
    end
  end
  def can_queenside_castle(board, :black) do
    case Enum.map([:a8, :e8], &Map.get(board, &1)) do
      [%{rank: :rook, color: :black, has_moved: false}, %{rank: :king, color: :black, has_moved: false}] -> true
      _ -> false
    end
  end
end
